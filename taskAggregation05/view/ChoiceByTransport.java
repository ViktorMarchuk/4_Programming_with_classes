...
public class ChoiceByTransport extends ChoiceByAllParameters{
    @Override
    public void getVoucher (Voucher[] vouchers){
        System.out.println("Enter searching by transport");
        String t = getScanner().next();
        boolean flag = true;
        for(int i = 0; i < vouchers.length; i++){
            if(vouchers[i].getTransfer().toLowerCase().contains(t)){
